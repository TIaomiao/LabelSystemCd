...
`apps/web` 已纳入 Git，但需要区分两部分：后来维护的 TypeScript 扩展有可编辑源码；原始旧 Viewer 的完整 React/Vite 源码已丢失，目前只有受版本控制的编译快照 `apps/web/legacy-runtime`。调查必须把这个边界作为已核实事实说明，不能笼统追问“apps/web 是否受版本控制”。对于复用现有批量清除、保存和重算调用链的小功能，应把工程实现量与轮廓数据风险分别评估。
