...
export const patchLegacyRuntime = (input) => {
  ...
  runtime = replaceOnce(
    ...
    'Zr=Yx(b==null?void 0:b.source),__pp=__cviPropagationSettings(b);window.__cviBuildDiagnosticSnapshot=()=>{const d=He?__cviContourAutoSaveState[He]:null,x=Array.isArray(Zt==null?void 0:Zt.exclude_regions)?Zt.exclude_regions.length:(Zt==null?void 0:Zt.exclude)?1:0;return{workstation_version:(Array.from(document.querySelectorAll("span,div")).map(E=>(E.textContent||"").trim()).find(E=>/^v\\d{4}\\.\\d{2}\\.\\d{2}/.test(E))||null),runtime_version:"2026.08.15-repro-fix-3",browser:navigator.userAgent,platform:navigator.platform,viewport:{width:window.innerWidth,height:window.innerHeight,device_pixel_ratio:window.devicePixelRatio},study_id:(o==null?void 0:o.id)??null,series_id:(j==null?void 0:j.id)??null,module:T??null,series_role:(j==null?void 0:j.role)??null,slice_index:_e,phase_index:ce,frame_key:br,tool_mode:Ie,contour_tool_mode:Se,selected_contour:c,exclude_region_count:x,autosave:{running:!!(d!=null&&d.running),pending:!!(d!=null&&d.nextTask)},latest_measurement_available:!!O,last_recompute_status:window.__cviLastRecomputeStatus??null,recent_actions:(window.__cviActionTrace??[]).slice(-20)}};async function __cviCopyDiagnostics(){const d=["CMR 问题反馈",`发生时间：${new Date().toLocaleString()}`,`页面/模块：CMR 勾画 / ${T??"未知"}`,"发生前操作：","预期结果：","实际结果：","出现次数：必现 / 偶发 / 第一次","是否刷新/换电脑：","诊断信息：",JSON.stringify(window.__cviBuildDiagnosticSnapshot(),null,2)].join("\\n");try{await navigator.clipboard.writeText(d),Bi("诊断信息已复制，可直接粘贴到反馈群。"),__cviRecordAction("diagnostics_copied",{module:T,series_id:(j==null?void 0:j.id)??null})}catch{window.prompt("请复制以下诊断信息：",d)}}',
